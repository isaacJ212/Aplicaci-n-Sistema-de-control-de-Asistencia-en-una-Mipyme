...
form.addEventListener('submit', async (e) => {
  ...
  try {
    // authApi.login envuelve el fetch y ya extrae data del ApiResponse
    const data = await authApi.login({ email, password });

    // data tiene: { token, refreshToken, expiration, email, role }
    ...
    if (code === 401) {
      ...
      // Errores de validación FluentValidation (array de mensajes)
